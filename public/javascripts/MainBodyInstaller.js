var MainBodyInstaller = function(vf,name,position){
	...
	this.installerSize = 50;
	createFrame(baseGroup,this.installerSize);
...
function buildController(machine){
	var controller={
						machine:machine,
						status:'stopped',//status:running/stopped
						loadStatus:'empty', //status:empty/loaded
						loadedObject:null
					};
...
function createFrame(baseGroup,size){
	//create base cube
...
function createCuboid(param){
	var segments = getSuitableSegments(4,param.width,param.length,param.height);
	var geometry = new THREE.BoxGeometry( param.width, param.length, param.height , segments.segmentsX, segments.segmentsY, segments.segmentsZ);
	var cube = new THREE.Mesh( geometry, new THREE.MeshPhongMaterial( { color: 0x22F5D9, overdraw: 0.5, transparent:true,opacity:0.5,shading: THREE.SmoothShading } ));
	cube.position.x = param.x;
	cube.position.y = param.y;
	cube.position.z = param.z;
